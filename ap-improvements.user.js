...
function getStorage() {
  ...
  for (const key of Object.keys(defa.cached)) {
    if (res.cached[key] !== undefined) continue;
    res.cached[key] = defa.cached[key];
  }
...
const translations = {
  default: {
    ...
    "title.button.bookmarks.layout.list": "Use list layout",
    "title.button.bookmarks.layout.grid": "Use grid layout",
    "title.button.bookmarks.share": "Share the bookmark list",
    "title.button.bookmarks.change_status": "Change bookmark watching status",
    "title.button.bookmarks.select_status": "Select anime watching status",
    "title.button.bookmarks.remove": "Remove this bookmark",
    "title.button.bookmarks.add": "Add bookmark",
    "title.button.index.change_filter_rule": "Change filter logic",
    "title.button.index.status_dropdown": "Choose status",
    "title.button.index.season_filter": "Set season filter",
...
.anitracker-big-list-item .anitracker-image-wrapper.anitracker-inline {
  display: inline-flex;
}
...
const siteVars = {
  ...
  cached: {
    ...
    poster: initialStorage.cached.poster,
    episodePage: [],
    animeId: {},
    animeSession: [],
    page: {},
...
async function getFirstEpisodeEntry(iinfo) {
  const cached = getCachedFirstEpisodeEntry(iinfo);
...
function getCachedFirstEpisodeEntry(iinfo) {
  if (!iinfo.id && !iinfo.session) return undefined;
  const found = (() => {
    for (const entry of siteVars.cached.firstEpisode) {
      if (iinfo.id === parseInt(entry[1], 36) && entry[1]) return entry;
      if (iinfo.session === entry[2] && entry[2]) return entry;
    }
  })();
...
function parseCachedFirstEpisodeEntry(entry) {
  return {
    first_episode: entry[0],
    id: parseInt(entry[1], 36),
    session: entry[2],
  };
}

function getCachedPoster(iinfo) {
  if (!iinfo.id && !iinfo.poster) return undefined;
  const entry = siteVars.cached.poster.find(g => (parseInt(g[1], 36) === iinfo.id && iinfo.id) || g[0] === iinfo.poster);
...
function updatePage() {
  ...
  if ($('.anitracker-view-notif-animes').length) {
    for (const item of $('.anitracker-notification-item')) {
      const entry = storage.notifications.episodes.find(a => a.animeId === +$(item).attr('anime-id') && a.episode === +$(item).attr('episode-data'));
...
const animeInfoFunctions = [
  {
    ...
    "fn": (iinfo = {}, config = {}) => {
      ...
      cachePoster(data?.poster, {...data}, getStorage());
  ...
  },
  {
    ...
    "instant": true,
    "fn": (iinfo = {}, config = {}) => {
      const found = getCachedPoster(iinfo);
      if (!found) return undefined;
      return {
        old: {
          poster: found.poster,
  ...
  {
    "id": "first_episode_entry",
    "outputs": ["first_episode","id","session"],
    "fn": async (iinfo = {}, config = {}) => {
      const firstEpEntry = await getFirstEpisodeEntry(iinfo);
      if (!firstEpEntry) return undefined;
      return {
        old: {
          session: firstEpEntry.session,
...
function openModal(title = '', backFunction, options = {}) {
  ...
  if (backFunction) close.replaceClass('fa-close', 'fa-arrow-left').attr('title', getText('title.button.modal.back'));
  ...
  window.onmessage = function(e) {
    ...
    if (action === 'request') {
      if (data.type === 'id') {
        ...
        cachePoster(pageData?.poster, pageData, getStorage());
...
function toUTCDate(timeString) {
  if (!timeString) return new Date(timeString);
...
function displayCollection(seriesList, elem) {
  ...
  $(`
  <a title="${toHtmlCodes(getText('title.search_result.collection', [seriesList[0].title]))}" href="javascript:;">
...
$('.input-search').attr('title',getText('title.input.site_search'));
...
<div class="anitracker-header">
  <button class="anitracker-header-notifications anitracker-header-button" title="${toHtmlCodes(getText('title.button.header.episode_feed'))}">
    <i class="fa fa-bell" aria-hidden="true"></i>
    <i style="display:none;" aria-hidden="true" class="fa fa-circle anitracker-header-notifications-circle"></i>
  </button>
  <button class="anitracker-header-bookmark anitracker-header-button" title="${toHtmlCodes(getText('title.button.header.bookmarks'))}">
...
function openNotificationsModal() {
  ...
  function openNotifAnimesModal(animation = true) {
    ...
    <label for="anitracker-week-start-dropdown-toggle" style="vertical-align: top;" title="${toHtmlCodes(getText('title.button.manage_feed.schedule.starting_day'))}">
      ${toHtmlCodes(getText('label.manage_feed.schedule.starting_day'))}
    </label>
    <div class="btn-group" style="margin-bottom: 10px;">
      <button class="btn dropdown-toggle btn-dark anitracker-flat-button" id="anitracker-week-start-dropdown-toggle" data-bs-toggle="dropdown" data-toggle="dropdown" title="${toHtmlCodes(getText('title.button.manage_feed.schedule.starting_day'))}">
        ${getDayName(0)}
      </button>
  ...
  function done(fromRefresh = true) {
    ...
    if (!storage.notifications.episodes.length) {
      $(`<div class="anitracker-center-content" style="height:75px;align-items:center;">
          <span>${toHtmlCodes(getText('info.episode_feed.empty'))}</span>
        </div>`)
  ...
  function addToList(num) {
    ...
    for (let i = currentNotificationIndex; i < storage.notifications.episodes.length; i++) {
      ...
      <div class="anitracker-big-list-item anitracker-notification-item${ep.watched ? "" : " anitracker-notification-item-unwatched"} anitracker-temp" anime-data="${data.id}" episode-data="${ep.episode}">
        <a href="/anitracker-redirect?s=${data.session}&a=${toHtmlCodes(encodeURIComponent(data.name))}&e=${ep.episode}" target="_blank" title="${toHtmlCodes(data.name)}">
          <div class="anitracker-image-wrapper anitracker-inline">
            <img src="${makePosterUrl(data.poster,'th')}" referrerpolicy="no-referrer" alt="${toHtmlCodes(getText('alt.thumbnail', [data.name]))}"}>
          </div>
          <i class="fa ${ep.watched ? 'fa-eye-slash' : 'fa-eye'} anitracker-watched-toggle" tabindex="0" aria-hidden="true" title="${toHtmlCodes(getText(ep.watched ? 'title.button.episode_feed.mark_episode.unwatched' : 'title.button.episode_feed.mark_episode.watched'))}"></i>
    ...
    $('.anitracker-notification-item.anitracker-temp .anitracker-watched-toggle').on('click keydown', (e) => {
      ...
      const animeId = +parent.attr('anime-id');
      ...
      elem.attr('title', getText(ep.watched ? 'title.button.episode_feed.mark_episode.unwatched' : 'title.button.episode_feed.mark_episode.watched'));
    ...
    for (const data of animeData) {
      ...
      $(elems[0]).addClass('anitracker-has-img-error-event');
      $(elems[0]).find('img').data('anime-data', data).on('error', function() {
        $(this).off('error');
        const oldData = $(this).data('anime-data');
        invalidatePosterCache(oldData.poster, getStorage());
        addImageSpinner(elems.find('.anitracker-image-wrapper'));
        getAnimeData(oldData, ["poster"], {requireNew: true}).then(newData => {
          ...
          const foundTemp = animeData.find(a => a.id === oldData.id);
          foundTemp.poster = newData.poster;
          if (found) {
            found.poster = newData.poster;
            saveData(storage);
          }
        });
      });
...
function openBookmarksModal() {
  ...
  <div style="display: flex;gap: 8px;flex-wrap: wrap;">
    ...
    <div class="btn-group anitracker-sort-method-btns">
      ...
      <button data-sort="alphabetical" class="btn btn-secondary" title="${toHtmlCodes(getText('title.button.bookmarks.sort.alphabetical'))}" ${sort === 'alphabetical' ? 'disabled' : ''}><i class="fa fa-sort-alpha-down" aria-hidden="true"></i></button>
      <button data-sort="status" class="btn btn-secondary" title="${toHtmlCodes(getText('title.button.bookmarks.sort.status'))}" ${sort === 'status' ? 'disabled' : ''}><i class="fa fa-list-alt" aria-hidden="true"></i></button>
    </div>
    <div class="btn-group anitracker-layout-btns">
      <button data-layout="list" class="btn btn-secondary" title="${toHtmlCodes(getText('title.button.bookmarks.layout.list'))}" ${layout === 'list' ? 'disabled' : ''}><i class="fa fa-list" aria-hidden="true"></i></button>
      <button data-layout="grid" class="btn btn-secondary" title="${toHtmlCodes(getText('title.button.bookmarks.layout.grid'))}" ${layout === 'grid' ? 'disabled' : ''}><i class="fa fa-th-large" aria-hidden="true"></i></button>
    </div>
    ${storage.bookmarks.length ?`
    <div style="flex-grow: 1;">
      <button class="btn btn-secondary anitracker-share-bookmarks-button" title="${toHtmlCodes(getText('title.button.bookmarks.share'))}"><i aria-hidden="true" class="fa fa-share"></i></button>
    </div>
    <div class="dropdown-menu anitracker-dropdown-content anitracker-share-bookmarks-dropdown" style="display:hidden;">
      <span>${toHtmlCodes(getText('label.dropdown.bookmarks.share'))}</span>
      <button title="${toHtmlCodes(getText('title.button.dropdown.bookmarks.share.image'))}" data-action="image"><i class="fa fa-image" aria-hidden="true"></i>${toHtmlCodes(getText('button.dropdown.bookmarks.share.image'))}</button>
      <button title="${toHtmlCodes(getText('title.button.dropdown.bookmarks.share.text'))}" data-action="text"><i class="fa fa-file" aria-hidden="true"></i>${toHtmlCodes(getText('button.dropdown.bookmarks.share.text'))}</button>
    </div>` : ''}
  </div>
  ...
  async function layoutEntries(storage = getStorage(), goToTop = true) {
    ...
    for (const g of entries) {
      ...
      if (layout === 'list') {
        ...
        <div class="anitracker-modal-list-entry anitracker-bookmark-list-entry" animeid="${g.id}" animename="${toHtmlCodes(name)}">
          ...
          <div>
            <button class="anitracker-bookmark-list-status anitracker-change-status-button" style="color:${statusAttrs[1]};" title="${toHtmlCodes(getText('title.button.bookmarks.change_status'))}">${toHtmlCodes(getStatusText(g.status))}</button>
        ...
        <div class="anitracker-modal-list-entry anitracker-bookmark-grid-entry" animeid="${g.id}" animename="${toHtmlCodes(name)}">
          ...
          <div style="display: flex;flex-direction: column;gap: 2px;">
            <button class="anitracker-bookmark-grid-status anitracker-change-status-button" style="color:${statusAttrs[1]};" title="${toHtmlCodes(getText('title.button.bookmarks.change_status'))}">${toHtmlCodes(getStatusText(g.status))}</button>
        ...
        else elem.find('img').on('error', function() {
          invalidatePosterCache(g.posterUrl, getStorage());
          getBookmarkImage(elem, g, session);
          $(this).off('error');
        });
      }
    ...
    Object.entries(statuses).forEach(([k,v]) => { $(`<button class="btn btn-dark anitracker-flat-button anitracker-status-button" title="${toHtmlCodes(getText('title.button.dropdown.bookmarks.status',[getStatusText(k)]))}" style="background-color: ${v[0]};width:100%;" data-value="${k}">
...
// imgWrapper cannot have inlined "display" styling
function addImageSpinner(imgWrapper) {
  imgWrapper.css('display','revert');
  $(`
  ...
  </div></div>`).prependTo(imgWrapper);
}

function removeImageSpinner(imgWrapper) {
  imgWrapper.css('display', '');
  imgWrapper.find('.anitracker-spinner-parent').remove();
}

function getBookmarkImage(elem, bookmark, session = undefined) {
  addImageSpinner(elem.find('.anitracker-big-poster-icon'));
  ...
  setTimeout(async () => {
    const data = await getAnimeData({
      ...
      id: bookmark.id,
      session: session,
    }, ['poster'], {ignored:['storage_bookmark'],requireNew:true});
    removeImageSpinner(elem.find('.anitracker-big-poster-icon'));
...
function openBookmarkStatusEditModal(id, adding=false) {
  ...
  <div style="display:flex;gap:10px;">
    <div class="anitracker-bookmark-edit-side" style="width:12em;max-width:12em;">
      ...
      <button class="btn dropdown-toggle btn-secondary anitracker-status-dropdown-button" data-bs-toggle="dropdown" data-toggle="dropdown" data-value="${status}" title="${toHtmlCodes(getText('title.button.bookmarks.select_status'))}" style="background-color: ${statusAttrs[0]};width:100%;display:block;margin-bottom:16px;">
  ...
  if (adding) {
    ...
    <button class="btn btn-secondary anitracker-flat-button anitracker-confirm-button" title="${toHtmlCodes(getText('title.button.bookmarks.add'))}" style="width:100%;" ${limitReached ? 'disabled' : ''}>
      <i class="fa fa-bookmark" aria-hidden="true"></i>
  ...
  imgElem.find('img').on('error', function() {
    invalidatePosterCache(entry.posterUrl, getStorage());
    getBookmarkImage(imgElem, entry, session);
    $(this).off('error');
  });
}
...
async function updateNotifications(animeName) {
  ...
  if (storage.settings.relativeEpNums && nobj.hasFirstEpisode && episodes.length) {
    cacheFirstEpisode(episodes[episodes.length - 1].episode, {
      id: nobj.id,
      session: data.session,
    }, storage);
  }
...
function loadIndexPage() {
  ...
  const texts = {
    changeRulesTitle: toHtmlCodes(getText('title.button.index.change_filter_rule')),
    statusDropdownTitle: toHtmlCodes(getText('title.button.index.status_dropdown')),
    seasonSelectorTitle: toHtmlCodes(getText('title.button.index.season_filter')),
    filterResultCounter: toHtmlCodes(getText('info.index.filter_results',['HTML'])),
    resetFiltersTitle: toHtmlCodes(getText('title.button.index.reset_filters')),
    resetFilters: toHtmlCodes(getText('button.index.reset_filters')),
    applyFiltersTitle: toHtmlCodes(getText('title.button.index.apply_filters')),
    applyFilters: toHtmlCodes(getText('button.index.apply_filters')),
    randomAnimeTitle: toHtmlCodes(getText('title.button.index.random_anime')),
    randomAnime: toHtmlCodes(getText('button.index.random_anime')),
    searchTitle: toHtmlCodes(getText('title.input.index.search')),
...
async function getAnimeDataFromSearch(iinfo = {}, config = {}) {
  ...
  cachePoster(rValue.poster, rValue, getStorage());
  ...
  $('#anitracker-continue-watching-show-more-button').on('click', () => {
    ...
    async function layoutEntries() {
      ...
      for (const entry of entries) {
        ...
        }, ["name","id","session","episode_session","poster","first_episode"], {requireInstant: true, ignored:['storage_video']});
        ...
        const episodeValue = getEpisodeValue(entry.episodeNum, undefined, data.first_episode);
        const titleAttr = `${toHtmlCodes(visibleAnimeName)}${data.episode_session ? ` Episode ${episodeValue}` : ''}`;
        ...
        <div class="anitracker-big-list-item anitracker-video-progress-item">
          ...
          <a href="${href}" target="_blank" class="anitracker-text-section" title="${titleAttr}">
            ...
            <span class="anitracker-normal-text">${toHtmlCodes(getText('generic.episode', [episodeValue]))}</span>
...
async function addContinueWatchingEpisodes(storage, episodeCount, clearAll = false) {
  ...
  for (const entry of videoTimes) {
    ...
    const firstEpisode = getCachedFirstEpisodeEntry({
      id: data.id ?? entry.animeId,
      session: data.session,
    })?.first_episode;
  ...
  function addEpElem(ep) {
    ...
    elem.find('img').on('error', function() {
      invalidatePosterCache(ep.snapshot, getStorage());
      const spinner = $(`
        <div class="anitracker-spinner anitracker-episode-spinner">
          <div class="spinner-border" role="status">
            <span class="sr-only">${toHtmlCodes(getText('screenreader.loading'))}</span>
      ...
      getAnimeData({
        ...
        name: ep.title,
      }, ["poster"], {requireNew: true}).then(data => {
        spinner.remove();
        if (!data?.poster) return;
        this.src = makePosterUrl(data.poster);
      });
    });
...
function updateContinueWatchingEpisodeValue(elem, firstEp) {
  const ep = elem.data('ep');
  const ep2 = elem.data('ep2');
  const epValue = getEpisodeValue(ep, ep2, firstEp);
...
async function getPageDataFromSession(session, isEpisode = false) {
  const page = await asyncGetPage(`/anime/${session}`);
  const data = page ? getAnimeDataFromPage(page, isEpisode) : undefined;
  if (!data) return undefined;
  cachePoster(data.poster, data, getStorage());
  return data;
}
